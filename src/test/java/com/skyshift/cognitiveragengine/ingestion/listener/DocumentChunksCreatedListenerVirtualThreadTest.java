...
@SpringBootTest
@ActiveProfiles("test")
class DocumentChunksCreatedListenerVirtualThreadTest {

    @Autowired
    private DocumentChunksCreatedListener documentChunksCreatedListener;

    @MockitoBean
    private ChunkVectorIngestionOrchestrator chunkVectorIngestionOrchestrator;

    @Test
    void onDocumentChunksCreated_runsOnVirtualThread() throws InterruptedException {
        ...
        doAnswer(invocation -> {
            ...
            latch.countDown();
            return null;
        }).when(chunkVectorIngestionOrchestrator).ingestVectorsAndIndexChunks(anyLong(), anyLong());

        documentChunksCreatedListener.onDocumentChunksCreated(new DocumentChunksCreatedEvent(1L, 2L));
